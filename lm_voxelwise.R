...
for (i in 1:length(opt)){
  if (is.na(opt)[i] == T) {
    ...
    cat('Use lm_voxelwise.R -h for an expanded usage menu.\n')
    quit()
  }
}
...
print("##############################################################################")
print("################       Linear Regression Voxelwise Script      ###############")
...
outsubDir <- paste0("n",dim(subjData)[1],"lm_Cov_",outName)
...
if (!residualMap) {
  ...
  model <- mclapply(m, function(x) {
    foo <- summary(lm(formula = x, data=subjData))
    residualVector <- lm(formula = x, data=subjData)$residuals
    
    return(list(foo$coefficients, residualVector))
  }, mc.cores = ncores)
...
for (j in 1:dim(model[[1]])[1]) {
  ...
  if (grepl("s(", variable, fixed=T)) {
    ...
    if (pAdjustMethod != "none") {
      writeNIfTI(pAdjustedOutImage,paste0("lmPadjusted_",pAdjustMethod, "_",var)) 
    }
    ...
    writeNIfTI(pOutImage,paste0("lmP_",var))
    writeNIfTI(zOutImage,paste0("lmZ_",var))
    writeNIfTI(tOutImage,paste0("lmT_",var))
    if (pAdjustMethod != "none") {
      writeNIfTI(pAdjustedOutImage,paste0("lmPadjusted_",pAdjustMethod, "_",var)) 
    }
  }
